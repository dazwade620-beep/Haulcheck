## Feature (2026-08 fork pt.29 — Movement Pack PDF, Shared Doc Alerts badge, Fuel Economy Leaderboard) — VERIFIED (testing_agent iter 51, backend 11/11 + frontend 100%)
Three features whose BACKEND already existed were finished by wiring the FRONTEND (+ small ranking sort):
1. **Movement Pack** — `GET /api/movements/pack?from_date=&to_date=&format=` (already built) returns one combined branded audit PDF of all border movements (or `format=json` summary). Frontend `Movements.js`: header **"Download pack"** button (`movements-pack-button`, shown only when items exist) → dialog (`movements-pack-dialog`) with From/To date pickers (`pack-from`/`pack-to`) + `pack-download-button` → `downloadPdf('/movements/pack…')`.
2. **Shared Doc Alerts** — backend `GET /api/global-docs/unseen-count` (uses `users.shared_docs_seen_at` vs `global_docs.created_at`) + `POST /api/global-docs/mark-seen` (already built). `Layout.js`: `sharedUnseen` polls unseen-count (60s) + listens for a `window 'shared-docs-seen'` event; red badge (`nav-shared-badge`) on the **Office** nav item when >0. `ComplianceDocs.js` SharedDocs `useEffect` calls `mark-seen` + dispatches the event on open → badge clears instantly.
3. **Economy Leaderboard** — `Fuel.js` (`FuelPanel`, rendered under **Fleet** route `/vehicles`, NOT Office): the per-vehicle list is now a ranked **Economy Leaderboard** (`fuel-league`) sorted by `avg_mpg` desc (nulls last). Each row (`fuel-vehicle-row`) has a rank badge (`fuel-rank`); rank 1 is amber-highlighted with a **"Most efficient"** badge (`fuel-top-badge`). Respects the existing date-range picker (`econ-from`/`econ-to`/`econ-clear`) which re-queries `/api/fuel/summary` and re-ranks.
- Verified: pack PDF 200/application/pdf; unseen-count/mark-seen flow; leaderboard TEST-FV1 rank1 (72.7 mpg, Most efficient) > TEST-FV2 rank2 (18.2 mpg). Testing agent left TEST_-prefixed seed data in DB.
- NOTE: PREVIEW verified — live (haulcheck.co.uk) needs Save-to-GitHub → Deploy.


## Feature (2026-08 fork pt.28 — Purchase odometer/MPG+L100km, Shared compliance library, GMR/HMRC + Border Movements page) — VERIFIED (testing_agent iter 50 + curl + screenshots)
Three requests delivered:
1. **Purchase odometer + real economy** — Vehicle/VehicleInput gained `purchase_odometer` (miles) + `purchase_date`. `_enrich_fuel(records, baselines)` seeds MPG from the purchase odometer (so the FIRST fill gets miles/MPG) and now also computes **L/100km**. `_fuel_baselines()` builds reg→purchase_odometer. `/api/fuel/summary` accepts `from_date`/`to_date` (date-range economy) and returns `avg_l_per_100km`; `/api/fuel/report` PDF gained an L/100km column. Frontend: Vehicles form has Purchase Date + Odometer at Purchase; Fuel tab shows MPG + L/100km per vehicle & in the fleet stat, plus a **date-range economy** bar (econ-from/econ-to/econ-clear). Verified curl: 100000→100600 = 600mi, 18.2 mpg, 15.5 L/100km.
2. **Shared compliance library (admin → all users)** — `GlobalDoc`/`GlobalDocInput` + `global_docs` collection. `GET /api/global-docs` (all authed users), `POST/DELETE /api/admin/global-docs` (require_admin), `GET /api/global-docs/files/{file_id}` (serves admin-uploaded files to any authed user, verifies file is referenced by a global doc). Frontend: `SharedDocs` component at top of Compliance Docs page — "Shared by HaulCheck" cards (read-only for normal users; admins get Add dialog + delete).
3. **GMR/HMRC + Border Movements** — OperatorInput gained `gmr_reference`; Operator page has the field + an **Open HMRC** button (GVMS login URL). NEW **Border Movements** page (`/movements`, sidebar "Movements" with Ship icon): `BorderMovement`/`BorderMovementInput` + `movements` collection; `GET/POST/PUT/DELETE /api/movements`. On create, GMR auto-fills from Company Settings if blank. Each movement is printable (`entry_reports.movement` → kind `movement`, `/api/print/movement/{id}`). Card + form fields: date, direction (import/export), GMR ref, vehicle, trailer, driver, route/crossing, ferry operator, status, notes, attachments; per-entry + global "Open HMRC" buttons.
- NOTE: PREVIEW verified — live (haulcheck.co.uk) needs Save-to-GitHub → Deploy.
- ⚠️ Editing note for future agents: rapid same-file edits during hot-reload caused a couple of write-race corruptions (orphaned duplicate tails / truncated lines) in Operator.js, Vehicles.js, ComplianceDocs.js, server.py — all fixed. Verify each edited file parses (babel/ast) after batches.


## Feature (2026-08 fork pt.27 — Bulk print, daily-check signatures, manager vehicle lock, feedback status/reply) — VERIFIED (curl + screenshots)
Four backlog items delivered together:
1. **Bulk Print "Print all for this vehicle"** — the per-vehicle combined pack already existed via `GET /reports/vehicle/{reg}` (PMI, service, wheel, defects, repairs, walkarounds, weekly, recalls, test history). Added a **Print** action + relabelled the Fleet page dropdown (`vehicle-history-print` → printPdf, plus Download PDF summary / full pack + evidence). New `printPdf()` helper in `frontend/src/lib/download.js`.
2. **Signature line on daily checks** — new `type:"signature"` section renderer in `pdf_export.build_report_pdf` (boxed Signature/Name/Date). `entry_reports.walkaround` now appends a "Sign-off" section with **Driver** + **Verified by (Transport/Fleet Manager)** boxes. Verified: `/api/print/walkaround/{id}` JSON shows Sign-off section; PDF 200.
3. **Manager Reassign / Lock** — added `vehicle_locked` to Driver + DriverInput + `_driver_profile`. Drivers page edit dialog has the assigned-vehicle select + a **"Lock to this vehicle"** checkbox (`drv-vehicle-lock`). `POST /driver/active-vehicle` returns **403** when locked. Driver app home shows a `driver-vehicle-locked-chip` (🔒 Locked, no Switch) instead of the Switch chip. Verified end-to-end.
4. **Feedback status + reply** (admin only) — `PATCH /api/feedback/{id}` (handled toggle) + `POST /api/feedback/{id}/reply` (emails submitter via Resend, marks handled, stores reply/replied_at). Feedback page (admin) shows an **Actioned** badge, **Mark actioned/Reopen** button, and a **Reply** dialog (`feedback-reply-*`). Verified via curl + screenshot.
- NOTE: PREVIEW verified — live (haulcheck.co.uk) needs Save-to-GitHub → Deploy.
...
- **Frontend** (driver/DriverApp.js): home reg chip is now a tappable **"Switch"** button (shows "Choose vehicle" when unassigned) → new `DriverSwitchVehicle` screen (`driver-switch-vehicle`) lists fleet vehicles (`switch-vehicle-{reg}`), highlights the current one, POSTs the choice, toasts "Now driving {reg}", and updates root `driver`/`profile` state so all screens reflect it instantly.
- Verified: `/driver/vehicles` deduped 5→4 with make/model; switch → `/driver/me` reflects new reg; invalid reg → 404; case-insensitive input normalises to fleet reg; UI switch AB12 CDE→BX20 XYZ updates chip + My Vehicle tile + toast. Test driver reset to AB12 CDE.
- NOTE: PREVIEW verified — live (haulcheck.co.uk) needs Save-to-GitHub → Deploy.
...
- **Backend** (server.py): models `DriverShift`, `DriverLocationInput`; collections `driver_shifts`, `driver_locations`. Endpoints:
  - Driver JWT: `POST /api/driver/shift/start` (ends any dangling active shift, opens new), `POST /api/driver/shift/end`, `GET /api/driver/shift/active`, `POST /api/driver/location` {lat,lng,accuracy?,speed?,heading?,recorded_at?,shift_id} → stores point with `date=recorded_at[:10]`.
  - Manager/admin (guard `require_tracking_view`: allows is_admin or role=="manager"; staff/viewer → 403): `GET /api/tracking/live` (each driver + on_shift + latest position), `GET /api/tracking/driver/{driver_id}?date=YYYY-MM-DD` (available `dates`, selected `date`, ordered `points`).
- **Driver PWA** (DriverApp.js `ShiftTracker`): "Start shift" button on home → requests geolocation → immediate first fix + `watchPosition` + a **30s heartbeat** (getCurrentPosition) so parked/stationary drivers still report; pings throttled to ≥20s apart; tagged with shift_id. Resumes an active shift on load via `/driver/shift/active`. "End shift" stops sharing. Shows points-sent + last time + permission errors.
- **Manager page** (Tracking.js, route `/tracking`, nav link `nav-tracking` shown only when role=="manager" || is_admin): left driver list (green pulse + "Live" badge when on shift, relative last-seen); click a driver → date <select> + point count + time span + map polyline (blue) with green start / red latest markers; live overview shows blue position markers for all drivers; auto-refresh every 30s; empty-state prompts drivers to tap Start Shift.
- Verified via curl: start shift → 3 pings → /tracking/live on_shift + last pos → /tracking/driver 3 points → end shift → off. Access control: staff→403, manager→200. Testing agent: all UI flows incl. mocked-geolocation Start/End Shift and e2e link — 100%, no bugs.
- Pre-existing data note (not a bug): manager@haulcheck.co.uk has a duplicate "John Smith" driver row.
- NOTE: PREVIEW only — needs Save-to-GitHub → Deploy to reach haulcheck.co.uk.
...
- **Calendar month-grid clarity** (Calendar.js): day numbers now bold + larger (w-7, text-sm, font-bold); events render as coloured status pills (new `eventBg()` helper: red-50/amber-50/slate-100) with a status dot + font-semibold text; cell min-h bumped to 132px. The click-to-enlarge day dialog (`dayOpen`, testids `day-dialog-events`/`day-dialog-add`) already existed and still opens with the full date title.
- **Training tab CPC Progress summary** (Training.js): new `cpc-progress-summary` card at the top of Office › Training. Reads `cpc_hours` + `cpc_expiry` per driver from GET /api/drivers, excludes left drivers and non-CPC drivers (no expiry AND 0 hours). Uses a PRO-RATA pace (user choice B): 5-year DQC cycle ending at `cpc_expiry`, expected = 35 × fraction-of-cycle-elapsed (~7h/yr); flagged `behind pace` (testid `cpc-behind-badge`) when logged hours are >0.5h below expected. Rows sorted most-behind first. Headline badge `cpc-behind-count` = "N behind pace" (amber) / "All on pace" (green). Each `cpc-summary-row` shows name, X/35h, bar, renewal date.
- **Training evidence button** (AuditReportDialog.js): the "PDF + evidence" button (testid `audit-download-evidence-button`) now shows for ANY report kind with `has_files` (was audit-only), so the standalone "Driver Training / CPC" report can print with attached certificates. Backend already merged training certificates via `_REPORT_FILE_KEYS["training"]`.
- Note: testing agent seeded 2 harmless CPC training records for Jane Doe in the manager@haulcheck.co.uk account.


## Fix/feature (2026-08 fork pt.13 — Training added to document generator) — VERIFIED (curl JSON+PDF+audit + UI screenshot)
- **Why training docs were missing**: the Fleet Audit Report generator (`AuditReportDialog` → `GET /api/reports/{kind}`) never had a `training` report kind, and the full `audit` pack didn't include training. It was a wiring gap, not a bug.
- **Added Driver Training / CPC report**: `reports.training_report(records, region)` (columns Driver, Course/Training, Category, Completed, Expiry, Hours, Provider; status from expiry). Registered in `_REPORT_BUILDERS["training"]`; `_report_data` fetches `db.training` and sets expiry-based status + date-filters on completed_date. Added to the `audit` pack kinds + `audit_pack` section + overview "Training records" count. Added `training` to `_REPORT_FILE_KEYS` (audit + standalone) so training attachments join the evidence bundle. Frontend dropdown option "Driver Training / CPC" added to `AuditReportDialog` REPORT_KINDS.
- Verified: `/api/reports/training?format=json` (2 rows, valid/expired statuses), PDF (application/pdf), and `/api/reports/audit` now contains the "Driver Training & CPC" section + overview count; UI dropdown renders the on-screen report.
